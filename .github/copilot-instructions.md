...
## 22. REFERÊNCIA COMPLETA: SCHEMA SUPABASE (40 TABELAS)

> **⚠️ FONTE DA VERDADE:** Esta é a referência oficial do schema do banco.  
> **Sempre consulte esta tabela antes de criar queries ou editar services.**

### 22.1 Tabela Consolidada: Todas as Colunas
...
### 22.2 Regras de Nomenclatura (PT ↔ EN)

**⚠️ CRÍTICO:** Frontend usa português (camelCase), Supabase usa inglês (snake_case).
...
- `entregadorId` → `driver_id`
- `tipoAtendimento` → `service_type`
- `statusEntrega` → `delivery_status`
- `dataHoraCriacao` → `created_at`
- `dataHoraConclusao` → `completed_at`

**Exemplo de conversão:**
```typescript
// Frontend (PT - camelCase)
const produto = {
  id: '123',
  nome: 'Gás P13',
  depositoId: 'dep-001',
...
### 22.3 Colunas REMOVIDAS (NÃO existem no schema)

**❌ NUNCA usar estas colunas em queries:**
- `products.current_stock` → Use `stock_balance.quantity`
- `products.min_stock` → Não existe
- `products.default_price` → Use `product_pricing.price`
- Qualquer campo `preco_padrao` → Use `sale_price` ou `product_pricing.price`

### 22.4 Campos Duplicados (Suporte Legacy)

Algumas tabelas têm campos duplicados para compatibilidade:
- `deposits.active` e `deposits.is_active` (use `is_active`)
- `employees.active` e `employees.is_active` (use `is_active`)
- `clients.active` e `clients.is_active` (use `is_active`)
- `products.active` e `products.is_active` (use `is_active`)

**Regra:** Sempre use `is_active` em novas features.
